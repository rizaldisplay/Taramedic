...
export const Topbar: React.FC<TopbarProps> = ({ onOpenSidebar }) => {
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-6 flex-shrink-0 z-10 w-full">
      {/* Kiri: Tombol Hamburger (Mobile/Tablet) & Breadcrumb */}
      <div className="flex items-center space-x-3 truncate mr-2">
        {/* Tombol Hamburger untuk memanggil Drawer Sidebar */}
        <button
        ...
        <div className="flex items-center text-sm text-slate-600 truncate">
          <span className="hidden sm:inline font-medium text-slate-800">EMR</span>
      ...
      <div className="flex items-center space-x-3 md:space-x-6">
        ...
        {/* Jam Realtime */}
        <div className="hidden xl:flex items-center text-slate-500 text-sm flex-shrink-0">
          <Clock className="w-4 h-4 mr-2" />
          Minggu, 16 Agustus 2026 19:10:30 WIB
        </div>
