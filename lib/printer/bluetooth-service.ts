 ...
 * unit tested or reused outside a hook. `useReceiptPrinter` wraps it.
 */
...
const DEFAULT_FILTERS: BluetoothLEScanFilterInit[] = [
  { namePrefix: 'RPP' },
  { namePrefix: 'Thermal' },
  { namePrefix: 'POS' },
...
export interface BluetoothServiceOptions {
  ...
  chunkDelayMs?: number;
  /**
   * Called whenever the connected device fires `gattserverdisconnected`.
   * Replaces the original's global `window.dispatchEvent(new CustomEvent('kiosk-printer-disconnected'))`
   * with a per-instance callback, which is friendlier to React's lifecycle.
   * The window event is still dispatched too, for any other app code relying on it.
   */
...
export class BluetoothService {
  ...
  private readonly onDisconnected?: (device: BluetoothDevice) => void;
  private disconnectHandler: ((event: Event) => void) | null = null;

  constructor(options: BluetoothServiceOptions = {}) {
    this.serviceUUID = options.serviceUUID ?? BLE_SERVICE_UUID;
    this.characteristicUUID = options.characteristicUUID ?? BLE_CHARACTERISTIC_UUID;
  ...
  async connect(device: BluetoothDevice, retries = 3): Promise<BluetoothRemoteGATTServer> {
    ...
    throw lastError instanceof Error ? lastError : new Error('Gagal koneksi Bluetooth');
  }
    ...
    this.disconnectHandler = (event: Event) => {
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('kiosk-printer-disconnected', {
            detail: { device: target },
          }),
        );
      }
  ...
  disconnect(device: BluetoothDevice | null): void {
    try {
      if (device?.gatt?.connected) {
