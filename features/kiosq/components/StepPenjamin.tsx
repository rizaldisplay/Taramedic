...
import { pilihPenjamin, submitPendaftaranKiosk } from '@/features/kiosq/slice/kioskSlice'; 
import { JenisPenjamin } from '@/types/common';

export const StepPenjamin: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
  const selected = useSelector((state: RootState) => state.kiosk.jenisPenjamin);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSelect = async (val: JenisPenjamin) => {
    if (isSubmitting) return;

    setIsSubmitting(true);
    dispatch(pilihPenjamin(val));
    await dispatch(submitPendaftaranKiosk(val));
    ...
    <div className="w-full max-w-2xl mx-auto text-center animate-in fade-in slide-in-from-right-8 duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        ...
        <button
          ...
          `}
        >
          {/* Main Area: Memisah antara Konten Normal vs State Loading */}
          <div className="flex-1 flex flex-col items-center justify-center w-full my-auto">
            {isSubmitting && selected === 'BPJS' ? (
              <div className="flex flex-col items-center justify-center gap-2 text-cyan-600">
                <Loader2 className="w-9 h-9 animate-spin" strokeWidth={2.2} />
          ...
          {/* Footer Note Card: Berada terpisah di paling bawah kartu */}
          <div className="w-full bg-slate-100/80 group-hover:bg-slate-200/60 rounded-xl p-2.5 sm:p-3 text-[11px] sm:text-xs text-slate-600 font-medium leading-relaxed transition-colors shrink-0">
